...
(function(root) {
    ...
    dust.log = function(message, type) {
        type = type || INFO;
        if (dust.debugLevel !== NONE && dust.indexInArray(loggingLevels,
            type) >= dust.indexInArray(loggingLevels, dust.debugLevel)) {
            if (!dust.logQueue) {
                dust.logQueue = [];
            }
            dust.logQueue.push({
    ...
    dust.render = function(name, context, callback) {
        var chunk = new Stub(callback)
            .head;
        try {
            dust.load(name, chunk, Context.wrap(context, name))
                .end();
        } catch (err) {
            chunk.setError(err);
        }
    ...
    dust.stream = function(name, context) {
        ...
        dust.nextTick(function() {
            try {
                dust.load(name, stream.head, Context.wrap(context, name))
                    .end();
            } catch (err) {
                chunk.setError(err);
            }
        ...
        return function(context, callback) {
            ...
            dust.nextTick(function() {
                if (typeof tmpl === 'function') {
                    tmpl(master.head, Context.wrap(context, name))
                        .end();
                } else {
                    dust.log(new Error('Template [' + name +
                        '] cannot be resolved to a Dust function'
                    ), ERROR);
                }
            });
            return master;
            ...
            if (dust.onLoad) {
                return chunk.map(function(chunk) {
                    dust.onLoad(name, function(err, src) {
                        ...
                        if (!dust.cache[name]) {
                            dust.loadSource(dust.compile(src,
                                name));
                        }
    ...
    dust.filters = {
        ...
        js: function(value) {
            if (!JSON) {
                dust.log(
                    'JSON is undefined.  JSON stringify has not been used on [' +
                    value + ']', WARN);
                return value;
            } else {
        ...
        jp: function(value) {
            if (!JSON) {
                dust.log(
                    'JSON is undefined.  JSON parse has not been used on [' +
                    value + ']', WARN);
                return value;
            } else {
    ...
    Context.prototype._get = function(cur, down) {
        ...
        if (typeof ctx === 'function') {
            ...
            if (ctx === undefined) {
                dust.log('Cannot find the value for reference [{' + down.join(
    ...
    Context.prototype.push = function(head, idx, len) {
        return new Context(new Stack(head, this.stack, idx, len), this.global,
            this.blocks, this.getTemplateName());
    };
    ...
    Context.prototype.getBlock = function(key, chk, ctx) {
        ...
        if (!blocks) {
            dust.log('No blocks for context[{' + key + '}] in template [' +
                this.getTemplateName() + ']', DEBUG);
            return;
        }
    ...
    Stub.prototype.flush = function() {
        ...
        while (chunk) {
            if (chunk.flushable) {
                ...
                dust.log('Chunk error [' + chunk.error +
                    '] thrown. Ceasing to render this template.', WARN);
    ...
    Stream.prototype.emit = function(type, data) {
        ...
        if (typeof handler === 'function') {
            ...
            dust.log('Event Handler [' + handler +
                '] is not of a type that is handled by emit', WARN);
        }
    };
    ...
    Stream.prototype.on = function(type, callback) {
        ...
        if (!this.events[type]) {
            dust.log('Event type [' + type +
                '] does not exist. Using just the specified callback.',
                WARN);
            if (callback) {
                this.events[type] = callback;
            } else {
                dust.log('Callback for type [' + type +
                    '] does not exist. Listener not registered.', WARN);
            }
    ...
    */
        ...
        dust.log('Not rendering section (#) block in template [' + context.getTemplateName() +
            '], because above key was not found', DEBUG);
        return this;
    };
    ...
    Chunk.prototype.exists = function(elem, context, bodies) {
        ...
        dust.log('Not rendering exists (?) block in template [' + context.getTemplateName() +
            '], because above key was not found', DEBUG);
        return this;
    };
    ...
    Chunk.prototype.notexists = function(elem, context, bodies) {
        ...
        dust.log('Not rendering not exists (^) block check in template [' +
            context.getTemplateName() + '], because above key was found',
            DEBUG);
        return this;
    };
    ...
    Chunk.prototype.partial = function(elem, context, params) {
        ...
        var partialChunk;
        if (typeof elem === 'function') {
            partialChunk = this.capture(elem, partialContext, function(name,
                chunk) {
                partialContext.templateName = partialContext.templateName ||
                    name;
                dust.load(name, chunk, partialContext)
                    .end();
            });
        } else {
